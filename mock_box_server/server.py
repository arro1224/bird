...
photos = [
    {"file_id": f"photo-demo-{index:03d}", "filename": f"DSC_{index:04d}.NEF", "format": "RAW", "captured_at": f"2026-07-12T06:{index % 60:02d}:{index % 60:02d}Z", "analysis_state": "completed" if index % 5 else "low_confidence", "thumb_ref": "", "preview_ref": "", "width": 6000, "height": 4000,
     "group_id": "group-demo-001" if index < 4 else "group-demo-002", "recognition": {"species_topn": [{"name": "白鹭" if index % 2 else "翠鸟", "confidence": 0.93 if index % 5 else 0.52}]}, "rating": {"total_score": round(5 + (index % 50) / 10, 1), "reason_tags": ["主体清晰", "眼部清晰"]}}
...
def payload(path: str, method: str, query: dict, body: dict):
    ...
    if path == "/api/v1/projects/current": return {"project_id": BATCH_ID, "name": "模拟晨拍批次", "created_at": "2026-07-10T06:00:00Z", "total_files": len(photos), "analyzed_count": len(photos), "review_count": 18, "keep_count": 9, "discard_count": 4, "pending_copy_count": 9, "copy_state": "pending"}
    ...
    if path == f"/api/v1/projects/{BATCH_ID}/groups":
        ...
        for group_id, group_type, member_ids, ranking, reasons in definitions:
            ...
            items.append({"group_id": group_id, "group_type": group_type, "representative_file_id": ranking[0], "member_file_ids": member_ids, "rank_order": ranking, "members": members, "recommendation_reasons": reasons, "captured_from": "2026-07-10T06:00:00Z", "captured_to": "2026-07-10T06:00:03Z"})
        return {"items": items}
    if path.startswith("/api/v1/files/"):
        ...
        if path.endswith("/decision") and method == "POST":
            ...
            version = len(history.setdefault(file_id, [])) + 1; body["version"] = version; decisions[file_id] = body; history[file_id].append({"version": version, "source": "app", "updated_at": "2026-07-11T00:00:00Z", "summary": "人工审阅已更新"}); return {"accepted": True, "version": version}
    ...
    if path == f"/api/v1/projects/{BATCH_ID}/copy" and method == "POST":
        ...
        copy_job_counter += 1
        jobs.append({"job_id": f"job-copy-{copy_job_counter:03d}", "job_type": "copy", "job_state": "running", "progress": 0.0, "total_count": 9, "finished_count": 0, "failed_count": 0, "current_file": ""}); return jobs[-1]
