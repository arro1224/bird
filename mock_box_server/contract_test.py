...
def main() -> None:
    ...
    check(all(data.startswith(b"\x89PNG\r\n\x1a\n") for data in server.PHOTO_MEDIA.values()), "gallery media must be valid PNG data")
    check(server.photos[0]["thumb_ref"].startswith("__MOCK_ORIGIN__"), "photo media should use a request-scoped origin marker")
    check(server.photos[0]["thumb_ref"].endswith(".png"), "gallery photos should use the generated bird assets")
