...
class TaskExperienceController extends ChangeNotifier {
  ...
  TaskSummary? get currentTaskOrNull {
    ...
    return taskById(currentTaskId);
  }

  TaskSummary get currentTask {
    final task = currentTaskOrNull;
    if (task == null) throw StateError('No task is currently available');
    return task;
  }
  ...
  /// Replaces presentation fixtures with repository-backed task snapshots.
  ///
  /// Production callers deliberately reject demo identifiers so a fallback
  /// fixture can never leak into the connected application.
  void replaceRepositoryTasks(Iterable<TaskSummary> tasks) {
    final next = List<TaskSummary>.unmodifiable(tasks);
    TaskSummary? demoTask;
    ...
    if (demoTask != null) {
      throw StateError(
        'Demo task ${demoTask.id} cannot enter the production task experience',
      );
    }
    _tasks = List.of(next);
  ...
  void replaceExecutableTaskTypes(Iterable<TaskType> values) {
    executableTaskTypes = List.unmodifiable(values);
    notifyListeners();
  }

  void setBusyState({bool? loading, bool? acting, Object? error, bool clearError = false}) {
    this.loading = loading ?? this.loading;
    this.acting = acting ?? this.acting;
  ...
  FutureOr<String> startImportBatch(String batchName) {
