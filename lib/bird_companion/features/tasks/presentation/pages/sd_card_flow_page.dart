...
class SdCardFlowPage extends StatelessWidget {
  ...
  final FutureOr<void> Function()? onRescan;
  ...
  Widget _content(BuildContext context) => switch (controller.sdCard.state) {
    ...
    SdCardReadState.scanning => _scanning(),
        ...
        child: Column(
          children: [
            _info(
              ...
              '照片数量',
              '${_count(controller.sdCard.photoCount)} 张',
            ),
            _info(
              Icons.pie_chart_outline_rounded,
              '预计读取空间',
              '${controller.sdCard.requiredSpaceGb.toStringAsFixed(1)} GB',
            ),
            _info(
  ...
  Future<void> _rescan(BuildContext context) async {
    final action = onRescan;
    if (action == null || controller.acting) return;
    try {
      await action();
    } catch (error) {
      if (context.mounted) {
