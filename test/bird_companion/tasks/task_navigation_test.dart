...
void main() {
  ...
  testWidgets('production task log callback reports the downloaded file', (tester) async {
    final controller = TaskExperienceController(const DemoTaskExperienceDataSource());
    addTearDown(controller.dispose);
    String? exportedTaskId;
    await tester.pumpWidget(
      MaterialApp(
        theme: AppTheme.light(),
        home: TaskDetailPage(
          ...
          onExportLog: (taskId) async {
            exportedTaskId = taskId;
            return r'C:\logs\task-diagnostics.zip';
          },
        ),
      ),
    );

    await tester.ensureVisible(find.text('导出日志'));
    await tester.tap(find.text('导出日志'));
    await tester.pump();

    expect(exportedTaskId, 'demo-copy-failed');
    expect(find.textContaining('task-diagnostics.zip'), findsOneWidget);
  });
